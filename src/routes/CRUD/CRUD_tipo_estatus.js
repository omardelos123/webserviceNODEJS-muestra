...
modulos.router.post('/CRUD_tipo_estatus', (req, res) => {
    console.log('CRUD_tipo_estatus');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('id_tipo_estatus', modulos.sql.Int, req.body.id_tipo_estatus);
                ...
                request.execute('CRUD_tipo_estatus').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
