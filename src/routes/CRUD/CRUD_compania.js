...
modulos.router.post('/CRUD_compania', (req, res) => {
    console.log('CRUD_compania');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('contacto', modulos.sql.VarChar(100), req.body.contacto);
                request.input('gerente', modulos.sql.VarChar(100), req.body.gerente);
                request.input('sub_gerente', modulos.sql.VarChar(100), req.body.sub_gerente);
                request.input('codigo_postal', modulos.sql.VarChar(100), req.body.codigo_postal);
                request.input('direccion', modulos.sql.VarChar(100), req.body.direccion);
                ...
                request.input('impuesto', modulos.sql.Numeric(18, 2), req.body.impuesto);
                request.input('pagina_web', modulos.sql.VarChar(100), req.body.pagina_web);
                request.input('descripcion', modulos.sql.VarChar(100), req.body.descripcion);
                ...
                request.execute('CRUD_compania').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
