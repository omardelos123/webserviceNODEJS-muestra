...
modulos.router.post('/CRUD_servicios', (req, res) => {
    console.log('CRUD_servicios');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('id_servicio', modulos.sql.Int, req.body.id_servicio);
                ...
                request.execute('CRUD_servicios').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
