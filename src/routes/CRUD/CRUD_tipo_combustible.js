...
modulos.router.post('/CRUD_tipo_combustible', (req, res) => {
    console.log('CRUD_tipo_combustible');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('id_tipo_de_combustible', modulos.sql.Int, req.body.id_tipo_de_combustible);
                ...
                request.execute('CRUD_tipo_de_combustible').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
