...
modulos.router.post('/CRUD_personal', (req, res) => {
    console.log('CRUD_personal');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('id_personal', modulos.sql.Int, req.body.id_personal);
                ...
                request.input('cedula', modulos.sql.VarChar(100), req.body.cedula);
                request.input('fecha_inicio', modulos.sql.Date, req.body.fecha_inicio);
                request.input('fecha_nacimiento', modulos.sql.Date, req.body.fecha_nacimiento);
                ...
                request.input('tipo_licencia', modulos.sql.VarChar(50), req.body.descripcion);
                request.input('foto_url', modulos.sql.VarChar(250), req.body.foto_url);
                ...
                request.input('foto', modulos.sql.Text, req.body.foto);
                request.input('tipo_imagen', modulos.sql.VarChar(100), req.body.tipo_img);
                request.input('id_clase_equipo', modulos.sql.Int, req.body.id_clase);
                request.execute('CRUD_personal').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
