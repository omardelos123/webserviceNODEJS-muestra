...
modulos.router.post('/CRUD_tipo_de_mantenimiento', (req, res) => {
    console.log('CRUD_tipo_de_mantenimiento');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.execute('CRUD_tipo_de_mantenimiento').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
