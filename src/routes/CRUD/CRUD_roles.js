...
modulos.router.post('/CRUD_roles', (req, res) => {
    console.log('CRUD_roles');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('nombre', modulos.sql.VarChar(100), req.body.nombre_rol);
                ...
                request.input('sw_crea', modulos.sql.Int, req.body.sw_crea);
                request.input('sw_eliminar', modulos.sql.Int, req.body.sw_eliminar);
                request.input('sw_editar', modulos.sql.Int, req.body.sw_editar);
                request.input('sw_deshabilita', modulos.sql.Int, req.body.sw_deshabilita);
                request.input('accion_tipo', modulos.sql.Char(1), req.body.accion_tipo);
                request.execute('CRUD_roles').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
