...
modulos.router.post('/CRUD_catalogo_de_compania', (req, res) => {
    console.log('CRUD_catalogo_de_compania');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
            modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('id_parte_compania', modulos.sql.Int, req.body.id_parte_compania);
                ...
                request.execute('CRUD_catalogo_de_compania').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
