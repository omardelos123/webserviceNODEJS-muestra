...
modulos.router.post('/CRUD_tipo_de_deficiencia', (req, res) => {
    console.log('CRUD_tipo_de_deficiencia');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('color', modulos.sql.VarChar(50), req.body.color);
                request.execute('CRUD_tipo_de_deficiencia').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
