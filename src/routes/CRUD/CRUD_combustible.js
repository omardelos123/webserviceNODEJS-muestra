...
modulos.router.post('/CRUD_combustible', (req, res) => {
    console.log('CRUD_combustible');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
            modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('id_orden_combustible', modulos.sql.Int, req.body.id_orden_combustible);
                request.input('cantidad', modulos.sql.Int, req.body.cantidad);
                request.input('costo', modulos.sql.Numeric(18, 2), req.body.costo);
                ...
                request.execute('CRUD_combustible').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
