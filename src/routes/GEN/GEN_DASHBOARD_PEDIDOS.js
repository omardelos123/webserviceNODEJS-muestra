...
modulos.router.post('/GEN_DASHBOARD_NUMERO_PARTE', (req, res) => {
    console.log('GEN_DASHBOARD_NUMERO_PARTE');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('top', modulos.sql.Int, req.body.top);
                request.execute('GEN_DASHBOARD_NUMERO_PARTE').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
