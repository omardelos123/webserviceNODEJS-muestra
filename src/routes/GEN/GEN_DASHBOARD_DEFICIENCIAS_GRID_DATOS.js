...
modulos.router.post('/GEN_DASHBOARD_DEFICIENCIAS_GRID_DATOS', (req, res) => {
    console.log('GEN_DASHBOARD_DEFICIENCIAS_GRID_DATOSaqui');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
            modulos.sql.connect(modulos.config, function (err) {
               ...
               // request.input('id_proveedor', modulos.sql.Int, req.body.id_proveedor);
                request.execute('GEN_DASHBOARD_DEFICIENCIAS_GRID_DATOS').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
