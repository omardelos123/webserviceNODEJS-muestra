...
modulos.router.post('/PROC_notificacion_ordenes', (req, res) => {
    console.log('PROC_notificacion_ordenes');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.execute('PROC_notificacion_ordenes').then(function (rows) {
