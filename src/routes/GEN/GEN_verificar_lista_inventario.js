...
modulos.router.post('/GEN_verificar_lista_inventario', (req, res) => {
    console.log('GEN_verificar_lista_inventario');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('nombre', modulos.sql.VarChar(100), req.body.nombre);
                request.execute('GEN_verificar_lista_inventario').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
