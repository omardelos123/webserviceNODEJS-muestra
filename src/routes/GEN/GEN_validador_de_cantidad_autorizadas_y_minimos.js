...
modulos.router.post('/GEN_validador_de_cantidad_autorizadas_y_minimos', (req, res) => {
    console.log('GEN_validador_de_cantidad_autorizadas_y_minimos');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.input('id_parte', modulos.sql.Int, req.body.id_proveedor);
                request.input('id_compania', modulos.sql.Int, req.body.id_compania);
                request.execute('GEN_validador_de_cantidad_autorizadas_y_minimos').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
