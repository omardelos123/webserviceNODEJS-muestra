...
modulos.router.post('/GEN_movimiento_de_inventario', (req, res) => {
    console.log('GEN_movimiento_de_inventario');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.execute('GEN_movimiento_de_inventario').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
