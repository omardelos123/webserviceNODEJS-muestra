...
modulos.router.post('/GEN_lista_proveedor_parte', (req, res) => {
    console.log('GEN_lista_proveedor_parte');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {    
            modulos.sql.connect(modulos.config, function (err) {
                ...
                request.execute('GEN_lista_proveedor_parte').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
