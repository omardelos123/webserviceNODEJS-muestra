...
modulos.router.post('/GEN_lista_parte_proveedor', (req, res) => {
    console.log('GEN_lista_parte_proveedor');
    if (!req.body.token) {
        res.status(401).send({
        ...
        if (valido == true) {
		    modulos.sql.connect(modulos.config, function (err) {
                ...
                request.execute('GEN_lista_parte_proveedor').then(function (rows) {
                    res.json(rows);
                    modulos.sql.close();
                }).catch(function (err) {
                    res.json(err);
